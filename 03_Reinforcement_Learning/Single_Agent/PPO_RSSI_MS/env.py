...
MANUAL_BPSK_POS  = np.array([400.0, 400.0])
MANUAL_QAM0_POS  = np.array([200.0, 500.0])
MANUAL_QAM1_POS  = np.array([550.0, 200.0])
# =============================================

OBS_SOFT_MARGIN = 130  # 반경(50) + 80
OBS_HARD_MARGIN = 70   # 반경(50) + 20
...
class DroneEnv:
    ...
    def _apf_repulsion(self, pos, velocity):
        ...
        우회 경로를 유도한다.
        """
        APF_INFLUENCE = 200.0    # OBS_RADIUS(50)+150: 더 멀리서부터 밀어냄
        APF_K_REP     = 720000.0  # 영향권 2배 → 계수 8배 (스케일 유지)
    ...
    def step(self, action):
        ...
        if curr_bpsk >= TARGET_RSSI_THRESHOLD:
            ...
            if curr_bpsk > 0.0:
                ...
                min_obs_dist = min(np.linalg.norm(self.drone_pos - obs)
                                   for obs in self.obstacles)
                obs_suppress = np.clip(
                    (min_obs_dist - OBS_RADIUS) / (OBS_RADIUS * 2.0), 0.3, 1.0)
