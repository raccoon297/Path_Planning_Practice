"""Run centralized Multi-Agent GWO and generate GWO artifacts."""
...
from optimizers.gwo import MultiAgentGWOConfig, run_multi_agent_gwo
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/gwo"),
        help="Directory for generated GWO artifacts.",
    )
    parser.add_argument("--wolves", type=int, default=80, help="Number of wolves.")
    parser.add_argument(
        "--iterations", type=int, default=150, help="Number of GWO iterations."
    )
...
def main() -> None:
    ...
    result = run_multi_agent_gwo(DEFAULT_SCENARIO, config, seed=args.seed)

    save_joint_plan_figure(
        result.plan,
        DEFAULT_SCENARIO,
        args.output_dir / "gwo_joint_plan.png",
        title="Multi-Agent GWO: Optimized Joint Plan",
    )
    save_fitness_convergence_figure(
        result.fitness_history,
        args.output_dir / "gwo_convergence.png",
        title="Multi-Agent GWO Best-so-far Objective",
    )
    if not args.skip_gifs:
        save_joint_motion_gif(
            result.plan,
            DEFAULT_SCENARIO,
            args.output_dir / "gwo_joint_motion.gif",
            title="Multi-Agent GWO: Synchronized Motion",
        )
        save_gwo_search_gif(
            result,
            DEFAULT_SCENARIO,
            args.output_dir / "gwo_search_evolution.gif",
        )
    ...
    (args.output_dir / "gwo_result.json").write_text(
        json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8"
    )

    print("=== Multi-Agent GWO ===")
    print(f"success: {result.success}")
    print(f"seed: {result.seed}")
    print(f"wolves: {config.num_wolves}")
